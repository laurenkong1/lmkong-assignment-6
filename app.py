...
def generate_plots(N, mu, sigma2, S):
    ...
    plt.savefig(plot1_path, bbox_inches='tight', dpi=100)
    plt.close()
    ...
    for _ in range(S):
        ...
        X_sim = np.random.uniform(0, 1, size=N).reshape(-1, 1) # Replace with code to generate X values

        # TODO: Generate Y values with normal additive error (mean mu, variance sigma^2)
        Y_sim = mu + np.random.normal(0, np.sqrt(sigma2), size=N)  # Replace with code to generate Y values
